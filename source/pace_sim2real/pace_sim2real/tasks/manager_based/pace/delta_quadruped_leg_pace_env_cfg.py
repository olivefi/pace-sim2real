...
##
# Physics backend — Kamino only (required for the leg's closed kinematic loops)
##


@configclass
class DeltaQuadrupedLegPacePhysicsCfg(PresetCfg):
    """Kamino-only physics preset, matched to the quadruped PACE/training preset.

    Identical solver settings to ``DeltaQuadrupedPacePhysicsCfg`` (``unified``
    collision pipeline, two solver substeps, default P-ADMM tolerances, 0.01
    contact margin) so the dynamics a single leg sees here match those it sees as
    part of the quadruped at training time.
    """
...
# loop closures open and the Kamino solver snaps them shut with a large impulse;
# this stance keeps both closures satisfied at spawn.
...
class DeltaQuadrupedLegPaceSceneCfg(PaceSim2realSceneCfg):
    ...
    robot: ArticulationCfg = DELTA_ALLREV_ALU_CFG.replace(
        ...
        actuators={
            ...
            "motors": DELTA_MOTOR_PACE_CFG.replace(max_delay=7, stiffness=20.0, damping=1.0),
...
class DeltaQuadrupedLegPaceCfg(PaceCfg):
    ...
    def __post_init__(self):
        ...
        self.bounds_params[:n, 1] = 1e-1  # armature [1e-3, 1.0] kg⋅m²
        self.bounds_params[n : 2 * n, 1] = 0.3  # viscous damping [0, 0.3] Nm⋅s/rad
        self.bounds_params[2 * n : 3 * n, 1] = 3.0  # static friction [0, 3] Nm
        self.bounds_params[3 * n : 4 * n, 1] = 3.0  # dynamic friction [0, 3] Nm
...
class DeltaQuadrupedLegPaceEnvCfg(PaceSim2realEnvCfg):
    ...
    def __post_init__(self):
        ...
        # Kamino-only physics, matched to the quadruped PACE/training preset.
