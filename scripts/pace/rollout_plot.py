...
def apply_params(articulation, sim_joint_ids, sim_params, device):
    ...
    viscous_friction_idx = slice(2 * n, 3 * n)
...
def main():
    ...
    with launch_simulation(env_cfg, args_cli):
        ...
        with torch.inference_mode():
            for step in range(num_steps):
                actions = target_dof_pos[step].unsqueeze(0)  # [1, n_joints]
                env.step(actions)
                sim_pos = articulation.data.joint_pos.torch[:, sim_joint_ids].squeeze(0)  # [n_joints]
    ...
    for i, name in enumerate(joint_order):
        ...
        ax.plot(
            ...
            color="grey", linestyle="--", linewidth=1.5, alpha=0.7, label="Target (real)",
        )
        ax.plot(
            time_np,
            measured_dof_pos[:, i].cpu().numpy(),
            color="tab:green", linestyle="--", linewidth=2, label="Measured (real)",
        )
        ax.plot(
            time_np,
            sim_dof_pos[:, i].numpy() - encoder_bias[i].item(),
            color="tab:orange", linewidth=2, label="Simulated",
        )
